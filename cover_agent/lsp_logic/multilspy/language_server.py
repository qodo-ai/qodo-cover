    ...
    async def request_completions(
        ...
        with self.open_file(relative_file_path):
            ...
            while response is None or (response["isIncomplete"] and num_retries < 30):
                ...
                response: Union[List[LSPTypes.CompletionItem], LSPTypes.CompletionList, None] = (
...
class SyncLanguageServer:
    ...
    def get_direct_context(self, captures, language, project_dir, rel_file):
        result = asyncio.run_coroutine_threadsafe(
            self.language_server.get_direct_context(captures, language, project_dir, rel_file),
            self.loop,
        ).result()
        return result
